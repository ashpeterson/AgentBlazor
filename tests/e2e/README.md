...
Override the production prompts with `AGENTBLAZOR_EXTERNAL_PROMPT`, `AGENTBLAZOR_EXTERNAL_SURFACE_PROMPT`, `AGENTBLAZOR_EXTERNAL_PANEL_PROMPT`, and `AGENTBLAZOR_EXTERNAL_BAR_PROMPT`. The default prompts cover explanation, operator checklist, safe next actions, and concise status-update flows so real production apps exercise more than a generic chat echo.
