...
Use this flow when you want to publish `AgentBlazor`, `AgentBlazor.Client`, and `AgentBlazor.Cli` privately and install them in a separate Blazor app as a test user.

## What This Gives You

- the package stays off `nuget.org`
- publishing is done from GitHub Actions
- a test user can install the runtime package, WebAssembly client package, and CLI tool from the GitHub Packages NuGet feed
...
5. Enter a prerelease version such as `0.1.0-preview.9`.
...
- `0.1.0-preview.9` has been validated from GitHub Packages against a clean Blazor Web App, the external real-world CSP/nonce-aware app `damienbod/BlazorSecurityNet10`, and a generated hosted WebAssembly server+client app using `AgentBlazor.Client`.
- `0.1.0-preview.9` includes the browser-safe `AgentBlazor.Client` package, hosted WebAssembly remote chat endpoint, SDK roll-forward, package-lock, and explicit web-app runtime framework updates needed for newer .NET 10 preview SDK environments.
- Use the same exact version for `AgentBlazor`, `AgentBlazor.Client`, and `AgentBlazor.Cli` when validating hosted WebAssembly apps. Scaffolded workflow files use `AgentBlazor.App` APIs such as `CapabilityResult`; those will not compile if the runtime package is stale or mismatched with the CLI.
- Published-feed validation for `0.1.0-preview.9` used isolated NuGet config/cache/tool paths, `agentblazor init`, `scaffold --diff`, `scaffold --approve`, `dotnet restore`, `dotnet build`, `doctor`, `validate`, runtime HTTP smokes with a placeholder `OpenAI__ApiKey`, and all-surface Playwright prompt validation against the external real-world app. Real-app report: `tests/e2e/artifacts/external-chat-widget/2026-04-20T17-33-11-449Z/report.md`. Hosted WebAssembly remote-client report: `tests/e2e/artifacts/hosted-wasm-remote-chat/2026-04-20T17-31-59-002Z/report.md`.
- To reproduce published-feed external chat validation from this repo, run `npm --prefix tests/e2e run test:external-chat-widget` with `AGENTBLAZOR_PACKAGE_SOURCE_MODE=published`, `AGENTBLAZOR_PACKAGE_VERSION=0.1.0-preview.9`, GitHub Packages credentials, `AGENTBLAZOR_EXTERNAL_REPO=https://github.com/damienbod/BlazorSecurityNet10.git`, `AGENTBLAZOR_EXTERNAL_PROJECT=BlazorApp/BlazorApp.csproj`, `AGENTBLAZOR_EXTERNAL_PROVIDER_MODE=deterministic`, and `AGENTBLAZOR_EXTERNAL_CHAT_SURFACES=widget,surface,panel,bar`.
- To reproduce published-feed hosted WebAssembly remote-chat validation from this repo, run `npm --prefix tests/e2e run test:hosted-wasm-remote-chat` with `AGENTBLAZOR_PACKAGE_SOURCE_MODE=published`, `AGENTBLAZOR_PACKAGE_VERSION=0.1.0-preview.9`, and GitHub Packages credentials.
