...
- current source and latest published-feed validated package are now `0.1.0-preview.8`; it includes SDK roll-forward support for newer .NET 10 preview SDKs, package-lock entries for `Microsoft.AspNetCore.App.Internal.Assets` `10.0.6`, and explicit `RuntimeFrameworkVersion` `10.0.6` pins for the demo and starter web apps
- GitHub Packages private-preview workflow now publishes both the runtime package and CLI tool package and uploads both package artifacts; workflow run `24597951350` passed for `0.1.0-preview.8` from commit `79cf68df3c448868d1e90a845d3629da20cb5672`
- published-feed real-app validation now proves `AgentBlazor` and `AgentBlazor.Cli` `0.1.0-preview.8` install from GitHub Packages, scaffold, restore, build, pass `doctor`/`validate`, render AgentBlazor assets in external Blazor apps including CSP/nonce-aware shells, and pass all-surface browser prompt validation
...
- latest source CI run `24598560893` is green; latest external chat matrix run `24586690690` is green across fresh Blazor no-provider, fresh Blazor deterministic, CleanArchitecture public widget, and CleanArchitecture authenticated widget/surface/panel/bar targets; latest published-feed all-surface external chat run `24598484039` is green against `damienbod/BlazorSecurityNet10`; latest real-usability nightly run `24598561465` is green
