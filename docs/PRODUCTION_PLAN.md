...
- current source and latest published-feed validated package are now `0.1.0-preview.9`; it includes the `AgentBlazor.Client` hosted WebAssembly remote-chat package, SDK roll-forward support for newer .NET 10 preview SDKs, package-lock entries for `Microsoft.AspNetCore.App.Internal.Assets` `10.0.6`, and explicit `RuntimeFrameworkVersion` `10.0.6` pins for the demo and starter web apps
- GitHub Packages private-preview workflow now publishes the runtime package, WebAssembly client package, and CLI tool package and uploads package artifacts; workflow run `24680658866` passed for `0.1.0-preview.9` from commit `faaeb6842ca90f4bd4cdeea070b1a28e30886463`
- published-feed real-app validation now proves `AgentBlazor` and `AgentBlazor.Cli` `0.1.0-preview.9` install from GitHub Packages, scaffold, restore, build, pass `doctor`/`validate`, render AgentBlazor assets in external Blazor apps including CSP/nonce-aware shells, and pass all-surface browser prompt validation
- published-feed hosted WebAssembly validation now proves `AgentBlazor.Client` `0.1.0-preview.9` installs from GitHub Packages into a generated server+client app and can prompt-test remote widget/surface/panel/bar through the server endpoint
- source external-app validation now covers all shipped chat surfaces, not only `AgentChatWidget`: the runner can inject a temporary route into reachable external app routes and prompt-test `AgentChatSurface`, `AgentChatPanel`, and `AgentChatBar` with production-style prompts
- latest publish workflow run `24680658866` is green; latest source CI run `24598560893` is green; latest external chat matrix run `24586690690` is green across fresh Blazor no-provider, fresh Blazor deterministic, CleanArchitecture public widget, and CleanArchitecture authenticated widget/surface/panel/bar targets; latest published-feed all-surface external chat validation report is green against `damienbod/BlazorSecurityNet10`; latest published-feed hosted WebAssembly remote-chat report is green; latest real-usability nightly run `24598561465` is green
...
- 2026-04-17 external chat surface matrix run `24586690690`: fresh `dotnet new blazor` no-provider and deterministic targets passed package install, scaffold idempotency, `doctor`, `validate`, widget minimize/Escape/reopen, and prompt submission for `AgentChatWidget`, `AgentChatSurface`, `AgentChatPanel`, and `AgentChatBar`; `neozhu/CleanArchitectureWithBlazorServer` public route passed the installed widget path; the authenticated users route passed widget/surface/panel/bar prompt submission with deterministic provider responses. AgentBlazor asset failure count was `0` in all reports.
- 2026-04-13 hosted WebAssembly candidate `https://github.com/davidfowl/TodoApp.git` at `307a1eadbbd77a3004c318f2377e4818bc400af6` was not used for scaffold validation because its `global.json` pins SDK `9.0.100`, while this environment has .NET SDK `10.0.106` installed.
...
- 2026-04-18 GitHub Packages workflow `publish-github-packages-preview` run `24597951350` passed for `0.1.0-preview.8` on commit `79cf68df3c448868d1e90a845d3629da20cb5672`. A clean Blazor Web App in `/tmp/agentblazor-published-preview8-clean-20260418053801` installed `AgentBlazor` and `AgentBlazor.Cli` from GitHub Packages, confirmed `agentblazor --version`, ran `init --non-interactive`, `scaffold --diff`, `scaffold --approve`, restore, build, `doctor` readiness `9/9`, and `validate` readiness `9/9`, validation `3/3`; runtime HTTP smoke rendered AgentBlazor CSS/JS assets. Published-feed real-app validation against `damienbod/BlazorSecurityNet10` at `682213b40201f3ce8000b10287492de4b6242d66` passed baseline restore/build, package and CLI install, scaffold, restore/build, `doctor` readiness `9/9`, `validate` readiness `9/9`, validation `3/3`, and CSP runtime smoke verified AgentBlazor CSS/JS assets carry the generated nonce. Workdir: `/tmp/agentblazor-published-preview8-damienbod-20260418053944/BlazorSecurityNet10/BlazorApp`.
- 2026-04-18 external chat workflow run `24598484039` passed published-feed all-surface validation on commit `069d9c48cfacf7f972eb2fa446939ab3dce961bc`. The run installed `AgentBlazor` and `AgentBlazor.Cli` `0.1.0-preview.8` from GitHub Packages into `damienbod/BlazorSecurityNet10`, then passed baseline build, scaffold, restore/build, `doctor` readiness `9/9`, `validate` readiness `9/9`, validation `3/3`, and Playwright prompt validation for `AgentChatWidget`, `AgentChatSurface`, `AgentChatPanel`, and `AgentChatBar` using deterministic provider responses and three production-style prompts per surface.
...
- [x] Set up package versioning (0.1.0-preview.9 in Directory.Build.props)
...
- [x] Run publish workflow with version 0.1.0-preview.8
- [x] Run publish workflow with version 0.1.0-preview.9
